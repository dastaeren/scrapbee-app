...
# BBS API import (optional - only needed for BBS platform)
try:
    from bbs_api import backfill_all_years, search_keywords
    BBS_AVAILABLE = True
except ImportError:
    BBS_AVAILABLE = False
...
def youtube_list_channel_video_ids(channel_id: str, api_key: str, max_items: int, timeout: int = 20) -> list:
    ids = []
    page_token = None
...
def youtube_video_details(video_ids: list, api_key: str, timeout: int = 20) -> list:
    details = []
...
def youtube_extract(first_line: str, max_items: int, settings: Settings) -> List[Dict[str, Any]]:
    ...
    ids = ids[:max_items]
    return youtube_video_details(ids, YOUTUBE_API_KEY, timeout=settings.timeout_seconds)
...
with tab_extract:
    ...
    # ── Platform dropdown (now includes BBS) ──────────────────────────────────
    platform = st.selectbox(
        "Platform",
        ["YouTube", "Generic Website (basic)", "BBS (All Years Archive)"],
        index=0
    )
    ...
    templates = {
        ...
        "BBS (All Years Archive)": ["Title", "Date", "URL", "Excerpt"]
    }
    ...
    # ── Platform-specific instructions + placeholder ──────────────────────────
    if platform == "YouTube":
    ...
    elif platform == "BBS (All Years Archive)":
        st.write("Input: **keywords** (one per line). Build archive once, then search instantly.")
        placeholder = (
    ...
    # ── BBS Archive build section (shown only when BBS platform selected) ─────
    DB_PATH = "bbs_archive.db"

    if platform == "BBS (All Years Archive)":
        st.markdown("#### BBS Archive (one-time setup)")

        if not BBS_AVAILABLE:
            st.error(
                "⚠️ `bbs_api.py` not found. Please make sure `bbs_api.py` is in the same folder as `app.py` "
                "and contains the functions `backfill_all_years` and `search_keywords`."
            )
        else:
            colx, coly = st.columns([1, 2])
            with colx:
                if st.button("Build / Update BBS Archive (All Years)", use_container_width=True):
                    st.session_state.stop_flag = False
                    prog_build = st.progress(0, text="Building BBS archive (all years)...")
                    try:
                        ...
                        prog_build.progress(100, text="BBS archive build completed.")
                        st.success(f"Archive ready ✅  Total posts stored: {total}")
                        log("OK", f"BBS archive updated. Total posts stored: {total}")
                    except Exception as e:
                        log("ERROR", str(e))
                        st.error(str(e))
                    finally:
            ...
            with coly:
                st.info("Build this once. After that, keyword search is instant and covers all years.")
        ...
        try:
            ...
            if not lines:
                ...
                # ── BBS ───────────────────────────────────────────────────────
                    ...
                    if not BBS_AVAILABLE:
                        st.error("bbs_api.py is not available. Cannot run BBS extraction.")
                    else:
                        keywords = [l.strip() for l in raw_input.splitlines() if l.strip()]
                        if not keywords:
                            st.warning("Please provide at least one keyword.")
                        elif not os.path.exists(DB_PATH):
                            st.warning("BBS archive not found. Click 'Build / Update BBS Archive (All Years)' first.")
                        else:
                            log("INFO", f"Searching BBS archive for keywords: {keywords}")
                            rows = search_keywords(DB_PATH, keywords, limit=max_items)
                            if not rows:
                                st.warning("No matches found in BBS archive.")
                            else:
                                for i, r in enumerate(rows, start=1):
                                    if stop_requested():
                                        log("WARN", "Stopped by user.")
                                        break
                                    st.session_state.extract_rows.append({c: r.get(c, "N/A") for c in columns})
                                    prog.progress(int((i / max(1, len(rows))) * 100), text=f"Matched {i}/{len(rows)}")
    ...
    if rows:
        ...
        # Build column_config — only add LinkColumn if "Video URL" or "URL" is present
        col_cfg = {}
        if "Video URL" in df_out.columns:
            col_cfg["Video URL"] = st.column_config.LinkColumn("Video URL", display_text="Open")
...
if not BBS_AVAILABLE:
    st.warning("bbs_api.py not found. BBS (All Years Archive) extraction will not work until bbs_api.py is placed in the same folder as app.py.")
